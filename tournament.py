...
def fetch_cricbuzz_scorecard(cb_match_id: str):
    """
    Fetch full scorecard using:
      1. cricbuzz-live.vercel.app  — unofficial proxy (still working as of 2026)
      2. cricketdata.org free API  — fallback (no key needed for basic scorecard)
    ...
    # ── Attempt 1: cricbuzz-live Vercel proxy ─────────────────────────────────
    # Full scorecard endpoint: /v1/scorecard/<id>
    url1 = f"https://cricbuzz-live.vercel.app/v1/scorecard/{cb_match_id}"
    try:
        r1 = requests.get(url1, headers=headers, timeout=12)
        if r1.status_code == 200:
            data = r1.json()
            result = _parse_vercel_proxy(data)
            if result.get("stats"):
                result["error"] = None
                return result
            errors.append(f"Proxy 200 but no stats (match not started or ID wrong)")
        else:
            errors.append(f"Proxy → HTTP {r1.status_code}")
    except Exception as e:
        errors.append(f"Proxy → {type(e).__name__}: {e}")

    # ── Attempt 2: Original Cricbuzz JSON API (may still work for some matches) ─
    url2 = f"https://www.cricbuzz.com/api/cricket-match/{cb_match_id}/full-commentary/0"
    try:
        r2 = requests.get(url2, headers={**headers, "Referer": "https://www.cricbuzz.com/"}, timeout=10)
        if r2.status_code == 200:
            data2 = r2.json()
            result2 = _parse_cricbuzz_json(data2)
            if result2.get("stats"):
                result2["error"] = None
                return result2
            errors.append(f"Cricbuzz JSON 200 but no stats")
        else:
            errors.append(f"Cricbuzz JSON → HTTP {r2.status_code}")
    except Exception as e:
        errors.append(f"Cricbuzz JSON → {type(e).__name__}: {e}")

    # ── Attempt 3: Cricbuzz live score endpoint (gives live 2 batsmen + 1 bowler) ─
    url3 = f"https://cricbuzz-live.vercel.app/v1/score/{cb_match_id}"
    try:
        r3 = requests.get(url3, headers=headers, timeout=10)
        if r3.status_code == 200:
            data3 = r3.json()
            result3 = _parse_vercel_live(data3)
            if result3.get("stats"):
                result3["error"] = "⚠️ Live score only (partial — full scorecard not yet available). " + " | ".join(errors)
                return result3
            errors.append(f"Live score → no data")
        else:
            errors.append(f"Live score → HTTP {r3.status_code}")
    except Exception as e:
        errors.append(f"Live score → {type(e).__name__}: {e}")

    return {"stats": {}, "raw_batting": [], "raw_bowling": [], "error": " | ".join(errors)}

def _parse_vercel_proxy(data):
    """Parse cricbuzz-live.vercel.app /v1/scorecard/<id> response."""
    stats = {}
    raw_bat, raw_bowl = [], []
    try:
        innings_list = data.get("data", {}).get("innings", [])
        if not innings_list:
            innings_list = data.get("innings", [])
        for inn in innings_list:
            # Batting
            for b in inn.get("batting", []):
                name = (b.get("batsman") or b.get("name") or "").strip()
                runs = int(b.get("runs") or b.get("r") or 0)
                diss = (b.get("dismissal") or b.get("how_out") or "").lower()
            ...
            for bw in inn.get("bowling", []):
                name = (bw.get("bowler") or bw.get("name") or "").strip()
                wkts = int(bw.get("wickets") or bw.get("wkts") or bw.get("w") or 0)
                if not name: continue
                if name not in stats: stats[name] = {"r":0,"w":0,"c":0.0,"s":0,"mom":0}
                stats[name]["w"] += wkts
...
def _parse_vercel_live(data):
    """Parse cricbuzz-live /v1/score/<id> — gives only 2 batsmen + 2 bowlers."""
    stats = {}
    raw_bat, raw_bowl = [], []
    try:
        d = data.get("data", data)
        for key in ["batsmanOne", "batsmanTwo"]:
            name = d.get(key, "").strip()
            runs_str = d.get(key.replace("batsman","batsmanOne").replace("One","OneRun").replace("Two","TwoRun")
                            if "One" in key else key.replace("batsmanTwo","batsmanTwoRun"), "0")
            if not name: continue
            try: runs = int(runs_str)
            except: runs = 0
            if name not in stats: stats[name] = {"r":0,"w":0,"c":0.0,"s":0,"mom":0}
            stats[name]["r"] = runs
            raw_bat.append({"name": name, "runs": runs, "dismissal": "(live)"})
        for bkey, wkey in [("bowlerOne","bowlerOneWickets"),("bowlerTwo","bowlerTwoWicket")]:
            name = d.get(bkey, "").strip()
            try: wkts = int(d.get(wkey, 0))
            except: wkts = 0
            if not name: continue
            if name not in stats: stats[name] = {"r":0,"w":0,"c":0.0,"s":0,"mom":0}
            stats[name]["w"] = wkts
...
def _credit_fielding(stats, diss):
    """Parse dismissal string and credit catches/stumpings/runouts."""
    if "c " in diss and "b " in diss:
        m = re.search(r"c\s+([\w\s]+)\s+b", diss)
        if m:
            fn = m.group(1).strip()
            if fn not in stats: stats[fn] = {"r":0,"w":0,"c":0.0,"s":0,"mom":0}
            stats[fn]["c"] += 1
    elif "st " in diss:
        m = re.search(r"st\s+([\w\s]+)\s+b", diss)
        if m:
            kn = m.group(1).strip()
            if kn not in stats: stats[kn] = {"r":0,"w":0,"c":0.0,"s":0,"mom":0}
            stats[kn]["s"] += 1
    elif "run out" in diss:
        m = re.search(r"run out\s*[\(\[]([\w\s/]+)[\)\]]", diss)
        if m:
            players = [x.strip() for x in re.split(r"[/&]", m.group(1)) if x.strip()]
...
def test_cricbuzz_connection():
    ...
    try:
        r = requests.get("https://cricbuzz-live.vercel.app/v1/matches",
                         headers={"User-Agent": "Mozilla/5.0"}, timeout=8)
        return True, f"✅ Cricbuzz proxy reachable — HTTP {r.status_code}"
    except Exception as e:
        return False, f"❌ Cannot reach proxy — {type(e).__name__}: {e}"
