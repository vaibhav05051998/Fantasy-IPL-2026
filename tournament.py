...
def load_db():
    # -----------------------------------------------------------------------
    # PLAYER MASTER — IPL 2026 VERIFIED SQUADS (cross-checked vs Wikipedia,
    # ESPNcricinfo, Olympics.com, Zee News full squad lists, Mar 2026)
    # KEY CHANGES FROM ORIGINAL:
    #   Sanju Samson      RR → CSK  (traded)
    #   Ravindra Jadeja   CSK → RR  (traded)
    #   Mohammed Shami    SRH → LSG (traded)
    #   Abdul Samad       SRH → LSG (traded)
    #   Aiden Markram     SRH → LSG (auction)
    #   Shahbaz Ahmed     SRH → LSG (auction)
    #   Matthew Breetzke  SRH → LSG (auction)
    #   Avesh Khan        RR  → LSG (retained by LSG)
    #   Mitchell Marsh    DC  → LSG (auction)
    #   Noor Ahmad        GT  → CSK (traded)
    #   Washington Sundar SRH → GT  (retained by GT)
    #   Prashant Veer     SRH → CSK (auction — most expensive uncapped)
    #   Kartik Sharma     IPL → CSK (auction)
    #   Ramakrishna Ghosh IPL → CSK (auction)
    #   Dewald Brevis     MI  → CSK (retained by CSK)
    #   Ayush Mhatre      MI  → CSK (retained by CSK)
    #   Sarfaraz Khan     DC  → CSK (auction)
    #   Anshul Kamboj     MI  → CSK (retained by CSK)
    #   Mukesh Choudhary  CSK → CSK ✓ (stays)
    #   Deepak Chahar     CSK → MI  (retained by MI)
    #   Sherfane Rutherford KKR→ MI (traded)
    #   Will Jacks        RCB → MI  (auction)
    #   Mitchell Santner  CSK → MI  (auction)
    #   Phil Salt         KKR → RCB (auction)
    #   Devdutt Padikkal  LSG → RCB (auction)
    #   Jitesh Sharma     PBKS→ RCB (auction)
    #   Tim David         MI  → RCB (auction)
    #   Romario Shepherd  MI  → RCB (auction)
    #   Suyash Sharma     KKR → RCB (auction)
    #   Cameron Green     RCB → KKR (auction — record buy)
    #   Lockie Ferguson   RCB → PBKS(auction)
    #   Blessing Muzarabani PBKS→KKR (replacement)
    #   Ajinkya Rahane    CSK → KKR (retained by KKR)
    #   Nitish Rana       KKR → DC  (traded)
    #   Umran Malik       SRH → KKR (auction)
    #   Finn Allen        RCB → KKR (auction)
    #   Tim Seifert       DC  → KKR (auction)
    #   Rahul Tripathi    SRH → KKR (auction)
    #   Harshal Patel     PBKS→ SRH (retained by SRH)
    #   Liam Livingstone  RCB → SRH (auction)
    #   Jos Buttler       SRH → GT  (retained by GT)
    #   Nehal Wadhera     MI  → PBKS(retained by PBKS)
    #   Azmatullah Omarzai GT  → PBKS(auction)
    #   Marcus Stoinis    LSG → PBKS(retained by PBKS)
    #   Ashutosh Sharma   PBKS→ DC  (retained by DC)
    #   David Miller      GT  → DC  (auction)
    #   Vipraj Nigam      RR  → DC  (auction)
    #   Lungi Ngidi       DC ✓ (stays)
    #   Mitchell Starc    DC ✓ (stays)
    #   Gudakesh Motie    GT  — removed (not in verified squads)
    #   Jacob Duffy       RCB — removed (not in verified squads)
    #   Quinton de Kock   LSG → MI  (auction)
    #   Ben Duckett       LSG — removed (not in verified squads)
    #   Donovan Ferreira  RR ✓ (traded from DC to RR)
    #   Himmat Singh      IPL → LSG (retained by LSG)
    #   Prithvi Shaw      DC  — listed but not in DC verified squad
    #   Bhuvneshwar Kumar RCB ✓ (stays)
    #   Vaibhav Suryavanshi RR ✓
    #   Cooper Connolly   MI  → PBKS (re-signed by PBKS per mykhel)
    #   Pathum Nissanka   IPL → DC  (retained by DC per Zee News)
    #   Jason Holder      IPL — removed (not in verified squads)
    #   Karun Nair        IPL → DC  (retained by DC per Wikipedia DC squad)
    # -----------------------------------------------------------------------
    pm = {
        # ── RCB ──────────────────────────────────────────────────────────────
        ...
        "David Miller":         {"team": "GT",  "role": "BAT",  "is_overseas": True},
