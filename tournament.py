...
<style>
  ...
  html, body, [data-testid="stAppViewContainer"] {
    background: #2a3550 !important; color: #f5f7ff !important;
  ...
  [data-testid="stAppViewContainer"] {
    background: radial-gradient(ellipse at 20% 0%, rgba(209,29,38,0.15) 0%, transparent 50%),
      ...
      linear-gradient(180deg, #2a3550 0%, #2e3a58 100%);
    background-attachment: fixed;
  }
  [data-testid="stHeader"] { background: transparent; }
  [data-testid="stSidebar"] {
    background: linear-gradient(160deg, #2e3d5c 0%, #283555 100%) !important;
    border-right: 2px solid rgba(255,215,0,0.30);
  }
  .main-title {
  ...
  .main-subtitle {
    ...
    font-size: 0.85rem; letter-spacing: 6px; color: rgba(255,215,0,0.70);
    text-transform: uppercase; margin-top: 4px; margin-bottom: 20px;
  }
  .pitch-divider {
    height: 3px;
  ...
  [data-testid="stTabs"] [role="tablist"] {
    gap: 4px; border-bottom: 2px solid rgba(255,215,0,0.3);
    background: rgba(0,0,0,0.15); border-radius: 8px 8px 0 0; padding: 4px 6px 0;
  }
  [data-testid="stTabs"] [role="tab"] {
    font-family: 'Bebas Neue', sans-serif; font-size: 1.1rem; letter-spacing: 2px;
    color: rgba(230,235,255,0.75) !important; border: none !important;
    border-radius: 6px 6px 0 0 !important; padding: 8px 20px !important;
  ...
  [data-testid="stTabs"] [role="tab"][aria-selected="true"] {
    color: #ffd700 !important; background: rgba(255,215,0,0.12) !important;
    border-bottom: 2px solid #ffd700 !important;
  }
  .player-card {
    background: rgba(255,255,255,0.10); border: 1px solid rgba(255,255,255,0.18);
    border-left: 3px solid #ccc; border-radius: 8px; padding: 8px 10px;
    margin-bottom: 6px; display: flex; align-items: center; gap: 10px;
    transition: border-color 0.2s, background 0.2s; min-height: 56px;
  }
...
def calc_points(scores_dict, match_ids):
    """
    Base: 1 run=1pt | 1 wkt=20pt | 1 catch/RO=5pt (0.5 RO = 2.5pt) | 1 stumping=5pt
    Bonus runs: >=50 → +5, >=100 → +10 (cumulative with base)
    Bonus wkts: >=3  → +5, >=5  → +10 (cumulative)
    MOM per match: +100 pts
    """
    total = 0.0
    for mid in match_ids:
        ...
        pts  = r * 1
        pts += w * 20
        pts += c * 5        # supports 0.5 → 2.5
        pts += st_ * 5
        pts += mom * 100    # MOM flag (0 or 1)
...
with t1:
    ...
    for p in sorted(pool):
        ...
        with cols[display_idx % 2]:
            ...
            with c_cell:
                st.markdown(f'''<div class="player-card" style="border-left-color:{team_color};">
    ...
    sq_c = "#ffd700" if len(final_squad)==11 else "#ff6b6b"
    os_c = "#ffd700" if os_count<=4 else "#ff6b6b"
    wk_c = "#ffd700" if wk_count>=1 else "#ff6b6b"
    bw_c = "#ffd700" if bowl_count>=4 else "#ff6b6b"
    st.markdown(f'''<div class="squad-status">
      <div class="stat-pill"><span class="stat-label">SQUAD</span><span class="stat-value" style="color:{sq_c};">{len(final_squad)}</span><span class="stat-max">/11</span></div>
      <div class="stat-pill"><span class="stat-label">✈️ OVERSEAS</span><span class="stat-value" style="color:{os_c};">{os_count}</span><span class="stat-max">/4 max</span></div>
      <div class="stat-pill"><span class="stat-label">🧤 KEEPERS</span><span class="stat-value" style="color:{wk_c};">{wk_count}</span><span class="stat-max">min 1</span></div>
      <div class="stat-pill"><span class="stat-label">🎳 BOWLERS</span><span class="stat-value" style="color:{bw_c};">{bowl_count}</span><span class="stat-max">/4 min</span></div>
    </div>''', unsafe_allow_html=True)
        ...
        with cols[i]:
            ...
            if s_data:
                ...
                for player in sorted(s_data["squad"]):
                    ...
                    st.markdown(f'<div class="squad-player-row"><span><span style="color:{t_color};font-size:9px;">●</span> {player} <span style="color:rgba(210,225,255,0.55);font-size:10px;">({info.get("team","")})</span></span>{cap_tag}</div>', unsafe_allow_html=True)
...
with t2:
    ...
    if sorted_lb:
        ...
        for rank, row in enumerate(sorted_lb[1:], start=1):
            ...
            gap_class = "score-gap-close" if gap <= 50 else ""
            gap_text  = f"▼ {gap} behind" if gap > 0 else "🔥 LEVEL!"
            st.markdown(f'''<div class="score-rival-row">
              ...
              <div class="score-gap-badge {gap_class}">{gap_text}</div>
            </div>''', unsafe_allow_html=True)
    ...
    st.markdown('<br>', unsafe_allow_html=True)
    st.caption("📡 Scores update live after admin pushes results. Half run-outs (0.5) score 2.5 pts. MOM = +100 pts.")
...
with t_admin:
    ...
    Run = 1pt &nbsp;|&nbsp; Wicket = 20pt &nbsp;|&nbsp; Catch/Full RO = 5pt &nbsp;|&nbsp; Half RO = 2.5pt (enter 0.5) &nbsp;|&nbsp; Stumping = 5pt &nbsp;|&nbsp; MOM = 100pt<br>
    <strong>Bonuses:</strong>&nbsp; 50+ runs → +5pt &nbsp;|&nbsp; 100+ runs → +10pt (replaces 50 bonus) &nbsp;|&nbsp; 3+ wkts → +5pt &nbsp;|&nbsp; 5+ wkts → +10pt (replaces 3 bonus)
    </div>""", unsafe_allow_html=True)
    ...
    # ── Collect all players in squads this week + manual search from full PM ──
    all_p_in_squads = set()
    for wk in db["selections"].values():
        for m_sel in wk.values(): all_p_in_squads.update(m_sel["squad"])

    # Players in squads whose team plays this match
    eligible_squad = [p for p in all_p_in_squads
                      if db["player_master"].get(p, {}).get("team") in teams]

    # Manual search: any player from the full PM matching the teams (for team corrections)
    st.markdown("**🔍 Add player by name** (search full player master if team assignment is wrong):")
    manual_search = st.text_input("Type player name to search", key="manual_search_admin", placeholder="e.g. Kohli, Bumrah...")
    manual_candidates = []
    if manual_search.strip():
        manual_candidates = [p for p in db["player_master"].keys()
                             if manual_search.lower() in p.lower()]
    if manual_candidates:
        manual_pick = st.selectbox("Select player to add to score entry:", ["— skip —"] + manual_candidates, key="manual_pick")
        if manual_pick and manual_pick != "— skip —" and manual_pick not in eligible_squad:
            eligible_squad.append(manual_pick)

    # ── MOM selection for this match ─────────────────────────────────────────
    st.markdown('<hr style="border:none;border-top:1px solid rgba(255,215,0,0.35);margin:8px 0 10px;">', unsafe_allow_html=True)
    st.markdown("**🏆 Man of the Match** (+100 pts to selected player)")
    ...
    selected_mom = st.selectbox("Select MOM for this match:", mom_options, index=mom_default, key=f"mom_{sel_mid}")
    st.markdown('<hr style="border:none;border-top:1px solid rgba(255,215,0,0.35);margin:8px 0 10px;">', unsafe_allow_html=True)
    ...
    for col, label, sub in [
        (hdr[1], "🏏 RUNS",     "1 run = 1pt | 50+=+5 | 100+=+10"),
        (hdr[2], "🎳 WICKETS",  "1 wkt=20pt | 3+=+5 | 5+=+10"),
        (hdr[3], "🤲 CATCH/RO", "1=5pt | 0.5 half-RO=2.5pt"),
        (hdr[4], "🧤 STUMPING", "1 = 5 pts"),
        (hdr[5], "PTS",         "live preview"),
    ]:
        col.markdown(f'<div style="font-family:\'Bebas Neue\',sans-serif;font-size:0.78rem;letter-spacing:1.5px;color:#ffd700;text-align:center;padding:4px 0 0;">{label}</div>'
                     f'<div style="font-size:9px;color:#aac0e0;font-family:\'Rajdhani\',sans-serif;text-align:center;padding-bottom:4px;">{sub}</div>',
                     unsafe_allow_html=True)
    ...
    if eligible_squad:
        for p in sorted(eligible_squad):
            ...
            cols[0].markdown(
                ...
                f'<span style="color:{t_color};font-size:10px;">●</span> <b style="color:#fff;">{p}</b>{mom_tag}<br>'
            ...
            live = r*1 + w*20 + c*5 + s*5 + mom_pts
            if r >= 100: live += 10
            elif r >= 50: live += 5
            if w >= 5: live += 10
            elif w >= 3: live += 5
            cols[5].markdown(f'<div style="font-family:\'Bebas Neue\',sans-serif;font-size:1.4rem;color:#ffd700;text-align:center;padding-top:6px;">{live:.1f}</div>', unsafe_allow_html=True)
    ...
    if st.button("🚀  PUSH SCORES", type="primary", use_container_width=True):
        # Clear old MOM for this match first, then set new one
