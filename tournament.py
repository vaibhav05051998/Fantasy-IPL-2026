...
def build_leaderboard(db, week_keys, active_week_name):
    lb = []
    lumpsum = db.get("lumpsum", {})   # {week_key: {manager: points}}
        ...
        for idx, wk_key in enumerate(week_keys):
            ...
            if sel:
                for p in sel["squad"]:
                    ...
                    w_sum += p_pts
            # Add any lump-sum points for this week/manager
            w_sum += float(lumpsum.get(wk_key, {}).get(mgr, 0))
            total += w_sum
...
with t2:
    ...
    rank_icons = {0:"🥇",1:"🥈",2:"🥉"}
    rows_html = ""
    lumpsum_any = any(db.get("lumpsum",{}).get(wk,{}) for wk in week_keys)
    for rank, row in enumerate(sorted_lb):
        icon = rank_icons.get(rank, f"#{rank+1}")
        # Calculate lump-sum total for this manager across all weeks
        ls_total = sum(float(db.get("lumpsum",{}).get(wk,{}).get(row["Manager"],0)) for wk in week_keys)
        ls_badge = f'<span style="font-size:9px;color:#90caf9;font-family:\'Rajdhani\',sans-serif;margin-left:6px;">+{round(ls_total,0):.0f} adj</span>' if ls_total > 0 else ""
        rows_html += (f"<tr><td style='text-align:center;font-family:\"Bebas Neue\",sans-serif;font-size:1.1rem;'>{icon}</td>"
                      f"<td style='font-weight:700;color:#eef2ff;'>{row['Manager']}{ls_badge}</td>"
...
with t_admin:
    ...
    # ══ LUMP-SUM WEEK SCORING ════════════════════════════════════════════════
    st.markdown('<div class="section-header">📅 LUMP-SUM WEEK SCORES</div>', unsafe_allow_html=True)
    st.markdown("""<div style="background:rgba(0,120,200,0.12);border:1px solid rgba(100,180,255,0.30);
        border-radius:8px;padding:10px 14px;margin-bottom:14px;font-family:'Rajdhani',sans-serif;
        font-size:13px;color:#cce4ff;">
        Use this when you missed entering player-level scores for a week.
        Enter the <b>total points for each manager directly</b> — these are added on top of
        any existing player scores for that week.
    </div>""", unsafe_allow_html=True)

    ls_week = st.selectbox(
        "Select week to enter lump-sum scores",
        list(SEASON_WEEKS.keys()),
        index=list(SEASON_WEEKS.keys()).index(active_week_name),
        key="lumpsum_week"
    )

    db.setdefault("lumpsum", {})
    db["lumpsum"].setdefault(ls_week, {})

    st.markdown(
        '<div style="font-family:\'Bebas Neue\',sans-serif;font-size:0.85rem;letter-spacing:2px;'
        'color:#ffd700;margin:8px 0 6px;">ENTER POINTS PER MANAGER</div>',
        unsafe_allow_html=True
    )

    ls_changed = False
    ls_cols = st.columns(len(db["pools"]))
    for i, mgr in enumerate(db["pools"].keys()):
        cur_ls = float(db["lumpsum"].get(ls_week, {}).get(mgr, 0))
        # Also show the player-based pts for reference
        sel = db["selections"].get(ls_week, {}).get(mgr, None)
        player_pts = 0.0
        if sel:
            for p in sel["squad"]:
                pp = calc_points(db["scores"].get(p, {}), list(SEASON_WEEKS[ls_week]["matches"].keys()))
                if p == sel["cap"]: pp *= 2
                player_pts += pp
        ...
        with ls_cols[i]:
            st.markdown(
                ...
                f'letter-spacing:2px;color:#ffd700;text-align:center;margin-bottom:2px;">{mgr}</div>'
                f'<div style="font-size:10px;color:rgba(200,220,255,0.55);text-align:center;'
                f'font-family:\'Rajdhani\',sans-serif;margin-bottom:4px;">'
                f'player pts: {round(player_pts,1)}</div>',
                unsafe_allow_html=True
            )
            new_ls = st.number_input(
                f"Lump pts",
                min_value=0.0, max_value=5000.0,
                value=cur_ls, step=0.5,
                key=f"ls_{ls_week}_{mgr}",
                label_visibility="collapsed"
            )
            if new_ls != cur_ls:
                db["lumpsum"][ls_week][mgr] = new_ls
                ls_changed = True
            total_wk = round(player_pts + new_ls, 1)
            st.markdown(
                f'<div style="font-size:11px;color:#a5d6a7;text-align:center;'
                f'font-family:\'Rajdhani\',sans-serif;font-weight:700;">total: {total_wk}</div>',
                unsafe_allow_html=True
            )

    if st.button("💾  SAVE LUMP-SUM SCORES", type="primary", use_container_width=True):
        save_db(db)
        st.success(f"✅ Lump-sum scores saved for {ls_week}!")
