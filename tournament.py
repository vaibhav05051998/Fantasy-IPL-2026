...
def fetch_cricbuzz_scorecard(cb_match_id: str):
    """
    Fetch scorecard from Cricbuzz JSON API + HTML fallback.
    Returns {"stats":{...}, "raw_batting":[...], "raw_bowling":[...], "error": str|None}
    Never returns None — always returns a dict so the caller can display the error.
    """
    headers = {
    ...
    # Attempt 1: JSON API
    url1 = f"https://www.cricbuzz.com/api/cricket-match/{cb_match_id}/full-commentary/0"
    try:
        r1 = requests.get(url1, headers=headers, timeout=10)
        if r1.status_code == 200:
            data = r1.json()
            result = _parse_cricbuzz_json(data)
            if result.get("stats"):
                result["error"] = None
                return result
            errors.append(f"JSON API 200 but no stats parsed (match may not have started)")
        else:
            errors.append(f"JSON API → HTTP {r1.status_code}")
    except Exception as e:
        errors.append(f"JSON API → {type(e).__name__}: {e}")

    # Attempt 2: HTML mobile scorecard
    ...
    try:
        ...
        if r2.status_code == 200:
            result = _parse_cricbuzz_html(r2.text)
            if result.get("stats"):
                result["error"] = None
                return result
            errors.append(f"HTML fallback 200 but no stats parsed")
        else:
            errors.append(f"HTML fallback → HTTP {r2.status_code}")
    except Exception as e:
        errors.append(f"HTML fallback → {type(e).__name__}: {e}")

    return {"stats": {}, "raw_batting": [], "raw_bowling": [], "error": " | ".join(errors)}

def test_cricbuzz_connection():
    ...
    try:
        ...
        return True, f"✅ Cricbuzz reachable — HTTP {r.status_code}"
    except Exception as e:
        return False, f"❌ Cannot reach Cricbuzz — {type(e).__name__}: {e}"
...
with t_admin:
    ...
    # ══ 1. EMERGENCY SQUAD OVERRIDE ══════════════════════════════════════════
    st.markdown('<div class="section-header">🚨 EMERGENCY SQUAD OVERRIDE</div>', unsafe_allow_html=True)
    st.markdown("""<div style="background:rgba(255,100,0,0.12);border:1px solid rgba(255,140,0,0.40);
        border-radius:8px;padding:10px 14px;margin-bottom:12px;font-family:'Rajdhani',sans-serif;
        font-size:13px;color:#ffe0b2;">
        ⚠️ Use this to restore lost squads or fix entries after the deadline.
        Bypasses the lock — admin only. Saves directly to DB.
    </div>""", unsafe_allow_html=True)

    override_mgr = st.selectbox("Select Manager to Override", list(db["pools"].keys()), key="override_mgr")
    o_pool       = db["pools"].get(override_mgr, [])
    existing_sel = db["selections"].get(active_week_name, {}).get(override_mgr, {"squad": [], "cap": ""})

    o_squad = st.multiselect(
        f"Select 11 players for {override_mgr}",
        options=sorted(o_pool),
        default=existing_sel.get("squad", []),
        key="override_squad"
    )

    # Validation indicators
    o_os   = sum(1 for p in o_squad if db["player_master"].get(p,{}).get("is_overseas"))
    o_wk   = sum(1 for p in o_squad if db["player_master"].get(p,{}).get("role")=="WK")
    o_bowl = sum(1 for p in o_squad if db["player_master"].get(p,{}).get("role")=="BOWL")
    o_cols = st.columns(4)
    o_cols[0].metric("Players", f"{len(o_squad)}/11", delta=None)
    o_cols[1].metric("Overseas", f"{o_os}/4 max", delta=None)
    o_cols[2].metric("Keepers", f"{o_wk} (min 1)", delta=None)
    o_cols[3].metric("Bowlers", f"{o_bowl} (min 3)", delta=None)

    o_cap_options = o_squad if o_squad else ["— pick squad first —"]
    o_cap_default = existing_sel.get("cap","")
    o_cap = st.selectbox(
        "Captain (2× points)",
        o_cap_options,
        index=(o_cap_options.index(o_cap_default) if o_cap_default in o_cap_options else 0),
        key="override_cap"
    )

    o_valid = len(o_squad)==11 and o_os<=4 and o_wk>=1 and o_bowl>=3 and o_cap in o_squad
    if st.button("💾  FORCE SAVE SQUAD", type="primary", use_container_width=True, disabled=not o_valid):
        if active_week_name not in db["selections"]:
            db["selections"][active_week_name] = {}
        db["selections"][active_week_name][override_mgr] = {"squad": o_squad, "cap": o_cap}
        save_db(db)
        st.success(f"✅ Squad saved for {override_mgr} — {o_cap} as captain.")
        st.rerun()
    if not o_valid and o_squad:
        st.warning("⚠️ Fix squad before saving: need exactly 11, ≤4 overseas, ≥1 keeper, ≥3 bowlers, captain selected.")
    ...
    # ══ 3. AUTO-FETCH WITH ERROR DISPLAY + TEST CONNECTION + CLEAR CACHE ════
    ...
    cb_id_input = st.text_input(
        ...
        value=default_cb_id,
        key=f"cbid_{sel_mid}",
        placeholder="e.g. 116484  (digits from cricbuzz.com URL)",
        help="Go to cricbuzz.com → open match scorecard → copy the number from URL"
    )
    ...
    # Test connection
    if test_clicked:
        with st.spinner("Testing connection to Cricbuzz..."):
            ok, msg = test_cricbuzz_connection()
        if ok:
            st.success(msg)
        else:
            st.error(msg)
            st.warning("Streamlit Cloud may be blocking outbound requests to Cricbuzz. Enter scores manually below.")

    # Clear cache
    if clear_clicked:
        st.cache_data.clear()
        st.success("✅ Cache cleared — click Fetch Scorecard again to retry.")
    ...
    if fetch_clicked and cb_id_input.strip():
        ...
        if result.get("error"):
            ...
            st.markdown(
                ...
                '💡 <b>If you see HTTP 403</b> → Cricbuzz is blocking the request. Use <b>Test Connection</b> to confirm, then enter scores manually.<br>'
                '💡 <b>If you see HTTP 404</b> → Match ID is wrong. Double-check the URL on cricbuzz.com.<br>'
                '💡 <b>If you see "no stats parsed"</b> → Match hasn\'t started yet or scorecard not available.<br>'
                '💡 <b>Try Clear Cache &amp; Retry</b> if you recently got an error and the match has since finished.'
                '</div>', unsafe_allow_html=True
            )
            ...
            if unmatched:
                st.markdown(f'<div style="font-size:11px;color:#ffa726;font-family:\'Rajdhani\',sans-serif;margin-top:6px;">⚠️ Unmatched (not in any squad): {", ".join(unmatched)}</div>', unsafe_allow_html=True)
